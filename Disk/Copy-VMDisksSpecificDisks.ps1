...
function Copy-VMDisks {
    ...
    .DESCRIPTION
        ...
        The Disk Map supports overwriting different disk numbers eg. Source Disk
    .EXAMPLE
    ...
    .EXAMPLE
        ...
        Copy-VMDisks -SourceVMName "VM1" -DestinationVMName "VM2" -DiskMapping $DisksMaps
    ...
    # Is a DiskMap being used?
    if ($PSBoundParameters.ContainsKey("DiskMapping")) {

        foreach ($DiskMap in $DiskMapping.GetEnumerator()) {

            Write-Host "Source Disk: $($DiskMap.key)" -NoNewline
            Write-Host " Destination Disk: $($DiskMap.value)"

            # Get disks
            $SourceDiskName = "Hard disk $($DiskMap.key)"
            $DestDiskName = "Hard disk $($DiskMap.value)"
            $SourceDisk = $SourceVM | Get-HardDisk | Where-Object {$_.Name -eq "$($SourceDiskName)"}
            $DestDiskPath = $DestinationVM | Get-HardDisk | Where-Object {$_.Name -eq "$($DestDiskName)"} | Select-Object -ExpandProperty Filename

            Write-ScreenAndLog -Message "Source path for $SourceDiskName is $($SourceDisk.Filename)..." -OutputPath $OutputPath
            Write-ScreenAndLog -Message "Destination path for $DestDiskName is $($DestDiskPath)...`n" -OutputPath $OutputPath
            Write-ScreenAndLog -Message "Copy Task request for $($SourceDiskName) to $($DestDiskName) Start Time: $(Get-Date -Format ("yyyy-MM-dd HH:mm:ss"))..." -OutputPath $OutputPath -Screen
        ...
        # Disks will be equal so should be same configuration
